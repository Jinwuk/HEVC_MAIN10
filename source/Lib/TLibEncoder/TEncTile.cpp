...
//	em_pcTilePredSearch->ETRI_destroy();		///em_pcTilePredSearchsTop 내부 메모리 해제 zeroone 20140311 : Useless???

	delete em_pcTileRdCost;   	 	em_pcTileRdCost    		= nullptr; 
	delete em_pcTileTrQuant;       	em_pcTileTrQuant    	= nullptr;    
	delete em_pcTilePredSearch; 	em_pcTilePredSearch	= nullptr; 
	delete em_pcTileCuEncoder;   	em_pcTileCuEncoder  	= nullptr; 
...
{
	TEncBinCABAC* 	e_pBinCABAC = nullptr; 
...
Void	TEncTile::ETRI_initTileCoders (TComPic* pcPic, TComSlice* pcSlice, TComRdCost* pcRdCost, TComTrQuant* pcTrQuant, TEncCavlc*& pcCavlcCoder, TEncSbac*& pcSbacCoder, TEncBinCABAC*& pcBinCABAC, TEncRateCtrl*& pcRateCtrl)
{
	...
	em_pcTileCuEncoder->ETRI_initForTiles(pcCavlcCoder, pcSbacCoder, pcBinCABAC, pcRateCtrl);	///Set CU Encoder for Tile Codings set Coders in Slice  
...
Void TEncTile::ETRI_RestoreRateControl(TComDataCU*& pcCU)
{
	if ( em_pcCfg->getUseRateCtrl() )
	{
		...
		actualQP = ( numberOfEffectivePixels == 0 )? g_RCInvalidQPValue : actualQP = pcCU->getQP( 0 );
		em_pcTileRdCost->setLambda(*em_pInfoCU->oldLambda);

		em_pcRateCtrl->getRCPic()->updateAfterLCU( em_pcRateCtrl->getRCPic()->getLCUCoded(), actualBits, actualQP, actualLambda,
...
Void TEncTile::ETRI_CompressUnit()
{
	...
	{
		*em_pInfoCU->uiCUAddr = uiCUAddr;				///Indicate the Cu Address to all Functions : 2015 5 20 by Seok

		// initialize CU encoder
		TComDataCU*& pcCU = em_pcPic->getCU( uiCUAddr );
		pcCU->initCU( em_pcPic, uiCUAddr );

		ETRI_InitRDCoderForSubStream(pcCU);				///Initilization Entropy Coder (TEncSBAC for CU Compression,	m_pcRDGoOnSbacCoder) @ 2015 5 15 by Seok
